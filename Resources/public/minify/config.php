...
if (isset($_GET['f']) && isset($_GET['base'])) {
    ...
    $_GET['f'] = '';
    $_SERVER['DOCUMENT_ROOT'] = ''; // Hotfix
    
    foreach ($f as $file)
    {
        if ($file != '') {
            $_GET['f'] .= dirname(__DIR__) . $_GET['base'] . $file . ","; 
        }
    }
...
$min_cachePath = sys_get_temp_dir() . '/minify';
...
/**
 * Maximum # of files that can be specified in the
 * Maximum # of files that can be specified in the "f" GET parameter
 */
$min_serveOptions['minApp']['maxFiles'] = 80;
